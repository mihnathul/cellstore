"""Harmonize MATR (Severson 2019) fast-charge LFP dataset to cellstore canonical schema.
...
def _build_cycle_frame(
    ...
    n = t_min.size
    if n == 0:
        return pd.DataFrame(), time_offset_s
    ...
    # Per-cycle signed charge swing (charge accumulated minus discharge removed).
    capacity_Ah = Qc - Qd

    df = pd.DataFrame({
        "test_id": test_id,
        "cycle_number": cycle_number,
        "step_number": pd.array([pd.NA] * n, dtype="Int64"),
...
def harmonize(ingested_data: dict, capacity_Ah: float = 1.1) -> dict:
    """Harmonize MATR ingested dict to canonical cellstore tables.
    ...
    for batch_cell, cell_payload in ingested_data.items():
        ...
        test_metadata.append({
            ...
            "cell_id": cell_id,
            "test_type": "cycling",
            "temperature_C_min": 30.0,                 # chamber set-point
            "temperature_C_max": 30.0,
            "temperature_step_C": 0.0,
            "soc_range_min": np.nan,
            "soc_range_max": np.nan,
            "soc_step": np.nan,
